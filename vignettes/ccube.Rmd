---
title: "Clustering cancer cell fractions"
author: "Ke Yuan"
date: "`r Sys.Date()`"
output: 
  html_document:
...
Now we overfit the data with 100 clones
```{r, cache=T}
K =100
res <- ccube_m6(mydata, epi=1e-3,
                init=K, tol = 1e-10, maxiter = 1e3, fit_mult = T, fit_hyper = T, verbose = F)
...
```
Estimated ccf and their weights. There are two clusters with vary low weights indicating no data is assigned to them. 
```{r}
res$mu
res$full.model$Epi
...
xx <- seq(range(mydata$ccube_ccf)[1],range(mydata$ccube_ccf)[2],  length.out = 1000)
ll <- 0
ll1 <- 0 
